...
//		initTerrainHeightMap();
		...
		private function initPlayer():void
		{
			...
			scene.addKinematicEntity(player);
...
//		initPlayerInternalControls();
		...
		private function drawRandomCircleOnBitmapData(bmd:BitmapData):void
		{
			...
			var x:Number = rand(radius, 4*radius);
			x += Math.random() > 0.5 ? 0 : bmd.width - x;
			var y:Number = rand(radius, 4*radius);
			y += Math.random() > 0.5 ? 0 : bmd.height - y;
...
//		gui.addToggle("cameraInputContext.mouseContext.enabled", {label:"mouse input"});
			...
			// player
			gui.addGroup("player");
			gui.addToggle("player.kinematicCapsuleVisible", {label:"debug kinematics"});
			gui.addToggle("player.dynamicCapsuleVisible", {label:"debug dynamics"});
